...
# Compute initial patient dataframe
patient <- priority()

# Identify patients with PriorityScore > 0.675
high_priority_patients <- patient$PatientName[patient$PriorityScore > 0.675]

# Load original data
original_data <- read.csv("Hackathon_Data1.csv", stringsAsFactors = FALSE)

# Set current date and time (formatted as in the dataset)
current_date <- "10-17-2025"
current_time <- "2214"

# Update the rows for high-priority patients
...
# Write the updated data to Hackathon_Data2.csv
write.csv(original_data, "Hackathon_Data2.csv", row.names = FALSE)

# Define priority_2 function (similar to priority but reads from Hackathon_Data2.csv)
priority_2 = function(){
  ...
  # Read the dataset
  data <- read.csv("Hackathon_Data2.csv", stringsAsFactors = FALSE)
...
# Compute the new patient dataframe
new_patient <- priority_2()

ppk = abs(0.675- mean(patient$PriorityScore))/(3*sd(patient$PriorityScore))
predicted_ppk = abs(0.675 - mean(new_patient$PriorityScore))/(3*sd(new_patient$PriorityScore))

ppk_set <- c(ppk = ppk, predicted_ppk = predicted_ppk)
return(ppk_set)
}